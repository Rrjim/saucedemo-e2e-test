...
export default class CheckoutInformationPage extends BasePage {
  ...
  async fillCheckoutInformation(
    ...
    world: {
      checkInfo?: { firstName: string; lastName: string; postalCode: string };
    }
  ): Promise<void> {
    await (
    ...
    const storedFirstName =
      await CheckoutInformationPageSelectors.firstName().getValue();
    const storedLastName =
      await CheckoutInformationPageSelectors.lastName().getValue();
    const storedPostalCode =
      await CheckoutInformationPageSelectors.postalCode().getValue();
    ...
    logger.info(
      `Checkout info stored in World object: ${JSON.stringify(world.checkInfo)}`
    );
  }
  ...
  async verifyFormSentSuccessfully(): Promise<void> {
    ...
    const isErrorDisplayed = await errorElement
      .isDisplayed()
      .catch(() => false);
    ...
    logger.info(
      `Form was sent successfully and Checkout Overview page is displayed.`
    );
  }

  async verifyCheckoutInfoNotEmpty(world: {
    checkInfo?: { firstName: string; lastName: string; postalCode: string };
  }): Promise<void> {
    if (!world.checkInfo) {
      throw new Error(
        "No checkout information found in World object to verify."
      );
    }
