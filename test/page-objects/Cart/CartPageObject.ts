...
export default class CartPage extends InventoryPage {
  ...
  async getItemQuantity(item: InventoryItem): Promise<number> {
    if (!item.quantity)
      throw new Error(`Quantity element missing for item "${item.name}"`);
  ...
  async verifyCartItemsMatchWorld(world: CustomWorld): Promise<void> {
    ...
    for (const expected of expectedItems) {
      ...
      expect(actual, `Item "${expected.name}" should be present in cart`).to
        .exist;
      ...
      if (actual) {
        ...
        const expectedPrice =
          typeof expected.price === "string"
            ? extractNumericPart(expected.price)
            : expected.price;

        expect(
          actual.description,
          `Description mismatch for "${expected.name}"`
