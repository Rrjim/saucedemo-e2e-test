...
export default class InventoryPage extends BasePage {
  ...
  async getAllItems(): Promise<InventoryItem[]> {
    ...
    for (const item of items) {
      const name = await (
        await InventoryPageSelectors.itemName(item)
      ).getText();
      const price = await (
        await InventoryPageSelectors.itemPrice(item)
      ).getText();
  ...
  async verifyItemCount(expectedItemsCount: string | number) {
    ...
    if (isNaN(expectedCount) || expectedCount < 0) {
      throw new Error(
        `Invalid number of products provided: ${expectedItemsCount}`
      );
    }
  ...
  async verifyAllPricesAreValid(): Promise<void> {
    ...
    numericPrices.forEach((price, index) => {
      expect(
        price,
        `Price for item "${items[index].name}" should be > 0`
      ).to.be.greaterThan(0);
    });
  }

  async verifyButtonsCountWithText(
    expectedText: string,
    ...
    const buttonTexts = await Promise.all(
      items.filter((item) => item.button).map((item) => item.button!.getText())
    );

    const actualCount = buttonTexts.filter(
  ...
  async removeAllItemsFromCart(): Promise<void> {
    ...
    expect(
      nonDefaultButtons.length,
      'All buttons should now say "Add to cart"'
    ).to.equal(0);
  }
}
