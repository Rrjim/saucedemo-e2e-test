...
export default class CheckoutOverviewPage extends CartPage {
   ...
   */
  async verifyPaymentAndShippingInfo(expectedInfo: Record<string, string>): Promise<void> {
    ...
    expect(actualPayment, 'Payment info mismatch').to.equal(expectedInfo.paymentInfo);
    expect(actualShipping, 'Shipping info mismatch').to.equal(expectedInfo.shippingInfo);
  }
  ...
  async verifyItemTotalAndTax(world: CustomWorld): Promise<void> {
    ...
    expect(actualTotal, 'Total after tax mismatch').to.equal(expectedTotalAfterTax);
